...
@Data
@ManagedBean(name = "bitClasificacionCofactor")
@ViewScoped
public class BitClasificacionCofactor implements Serializable {

  private static final Logger log = LogManager.getLogger(BitClasificacionCofactor.class);
  ...
  private String nombreArchivo = "BitacoraClasCofactor";

  private List<FiBitacoraImx> listaBitacoraClasCofactor;
  ...
  public BitClasificacionCofactor() {
    usuarioLogueado = ValidadorSesion.getUsuarioLogueado();
    if (usuarioLogueado != null) {
      ValidadorSesion.validarPermiso("BitacoraClasificacionDeCofactor", usuarioLogueado.getRol());
    }
    init();
  }
  ...
  private void init() {
    ...
    llenarClientes();
    llenarListaBitacoraClasCofactor();
  }

  public void filtarTableBitClasCofactor() {
  ...
  private String condicionMesAnio() {
    ...
    return " TIPO_PROCESO = 'COF' AND ATRIBUTO3 = '" + mes + "' AND ATRIBUTO4 = '" + anio + "' ";
  }

  @SuppressWarnings("unchecked")
  private void llenarListaBitacoraClasCofactor() {
    log.debug("Entramos a llenarListBitacoraClasCofactor");

    listaBitacoraClasCofactor = genericService.get("FiBitacoraImx",
        condicionMesAnio() + ((filtroCampoA != null && !filtroCampoA.equals("-1")) ?
            " AND CAMPOA = NVL('" + filtroCampoA + "',CAMPOA) )" : ""), " ORDER BY CAMPOA, CUENTA_ORIGEN, MONTO_SICAV");

    for (final FiBitacoraImx bit : listaBitacoraClasCofactor) {
      bit.setMontoSicavFor(Utilerias.formatearNumero(bit.getMontoSicav(), ""));
      bit.setMontoCofFor(Utilerias.formatearNumero(bit.getMontoCofactores(), ""));
    }
    paginatorVisible = (listaBitacoraClasCofactor.size() > 16);
    exportarBitacoraClasCofactor();
  }
  ...
  public void exportarBitacoraClasCofactor() {
    ...
    try (final HSSFWorkbook wb = new HSSFWorkbook()) {
      ...
      cell.setCellValue("BITACORA CLASIFICACION COFACTOR");
      cell.setCellStyle(style);

      final HSSFRow row = sheet.createRow((short) 1);
      row.createCell(0).setCellValue("Contrato IMX");
      row.createCell(1).setCellValue("CampoA");
      row.createCell(2).setCellValue("Nombre Cliente");
      row.createCell(3).setCellValue("Cuenta");
      row.createCell(4).setCellValue("Saldo Sicav");
      row.createCell(5).setCellValue("Campo B");
      row.createCell(6).setCellValue("Saldo Cofactor");

      final DecimalFormat decimalFormat = new DecimalFormat("###,###.00");

      int k = listaBitacoraClasCofactor.size() + 2;
      for (FiBitacoraImx bitacora : listaBitacoraClasCofactor) {
        k--;
        final HSSFRow row2 = sheet.createRow((short) k);
        row2.createCell(0).setCellValue(bitacora.getContrato());
        row2.createCell(1).setCellValue(bitacora.getCampoA());
        row2.createCell(2).setCellValue(bitacora.getNomCliente());
        row2.createCell(3).setCellValue(bitacora.getCuentaOrigen());
        row2.createCell(4).setCellValue(bitacora.getMontoSicav() != null ?
            decimalFormat.format(bitacora.getMontoSicav()) : "0.0");
        row2.createCell(5).setCellValue(bitacora.getCampoB());
        row2.createCell(6).setCellValue(bitacora.getMontoCofactores() != null ?
            decimalFormat.format(bitacora.getMontoCofactores()) : "0.0");
      }
      ...
      Utilerias.guardarMensajeLog("BitacoraClasificacionCofactor", "exportarBitacoraClasificacionCofactor",
          ex, "Error Creando Archivo Excel", usuarioLogueado);
    }
  }
