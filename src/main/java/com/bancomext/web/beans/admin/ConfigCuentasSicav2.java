...
@Data
@ManagedBean(name = "configCuentasSicav2")
@ViewScoped
public class ConfigCuentasSicav2 implements Serializable {

  private static final Logger log = LogManager.getLogger(ConfigCuentasSicav2.class);
  ...
  private void init() {
    setListaComparativo(genericService.get("FiCatCuentasSicav", " 1=1 ", " Order by id.cta desc "));
    setPaginatorVisible(getListaComparativo().size() > 6);
  }

  @SuppressWarnings("unchecked")
  public void consultar() {
    StringBuilder condicion = new StringBuilder();
    condicion.append(cuentaF != null && !cuentaF.isEmpty() ? " id.cta = '" + cuentaF + "' @" : "");
    condicion.append(subcuentaF != null && !subcuentaF.isEmpty() ? " id.scta = '" + subcuentaF + "' @" : "");
    condicion.append(subsubcuentaF != null && !subsubcuentaF.isEmpty() ? " id.sscta = '" + subsubcuentaF + "' @" : "");
    condicion.append(subsubsubcuentaF != null && !subsubsubcuentaF.isEmpty() ? " id.ssscta = '" + subsubsubcuentaF + "' @" : "");
    String[] con = condicion.toString().split("@");
    condicion = new StringBuilder();
    for (int i = 0; i < con.length; i++) {
      condicion.append(i == 0 ? con[i] : " and " + con[i]);
    }
    setListaComparativo(genericService.get("FiCatCuentasSicav", condicion.toString(), " Order by id.cta desc "));
    setPaginatorVisible(getListaComparativo().size() > 6);
  }
